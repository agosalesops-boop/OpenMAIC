...
import { ArrowRight, ShieldCheck, LoaderCircle, Copy, Check } from 'lucide-react';
import type { AccessRole } from '@/lib/server/access-token';

interface AccessCodeModalProps {
  open: boolean;
  onSuccess: (role: AccessRole, name?: string | null) => void;
}
...
export function AccessCodeModal({ open, onSuccess }: AccessCodeModalProps) {
  ...
  const [bootstrap, setBootstrap] = useState<{ role: AccessRole; name?: string | null; code: string } | null>(
    null,
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        const data: { role?: AccessRole; name?: string | null; bootstrapCode?: string } = await res
          .json()
          .catch(() => ({}));
        const role: AccessRole = data.role === 'learner' ? 'learner' : 'admin';
        if (data.bootstrapCode) {
          // First-ever login on this deployment: this admin account was just
          // created from the shared bootstrap code, which will never work
          // again. Show the freshly generated personal code once before
          // continuing -- it can't be recovered later.
          setBootstrap({ role, name: data.name, code: data.bootstrapCode });
          setLoading(false);
          return;
        }
        setSuccess(true);
        setTimeout(() => onSuccess(role, data.name), 600);
      } else {
  ...
  async function handleCopyBootstrapCode() {
    if (!bootstrap) return;
    try {
              ...
              {bootstrap ? (
                <>
                  ...
                  <h1 className="mb-1 text-center text-lg font-semibold tracking-tight text-foreground">
                    {t('accessCode.bootstrapTitle')}
                  </h1>
                  <p className="mb-6 text-center text-sm text-muted-foreground">
                    {t('accessCode.bootstrapNotice')}
                  </p>
                  <div className="mb-6">
                    <p className="mb-1.5 text-xs font-medium text-muted-foreground">
                      {t('accessCode.bootstrapCodeLabel')}
                    </p>
                    <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-background/60 px-4 py-3">
                      ...
                      <button
                        type="button"
                        onClick={handleCopyBootstrapCode}
                  ...
                  <button
                    type="button"
                    onClick={() => onSuccess(bootstrap.role, bootstrap.name)}
